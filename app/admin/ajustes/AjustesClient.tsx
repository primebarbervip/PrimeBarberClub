...
export default function AjustesClient({ config: initialConfig }: AjustesClientProps) {
    ...
    const handleSaveBranding = async () => {
        ...
        formData.append("nombreTienda", branding.nombreTienda);
        formData.append("logo", branding.logo);
        // Keep internal values to avoid clearing them
        ...
        if (result.success) {
            showToast('success', "Identidad visual actualizada correctamente");
            // Trigger a simple reload to refresh metadata if needed (or revalidate works via actions)
        } else {
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                    <div className="md:col-span-8 space-y-6">
                        <div className="space-y-2">
                            <label className="text-[9px] font-black uppercase tracking-widest text-zinc-500 ml-1">Título de la Web</label>
                            <input
                                type="text"
                                value={branding.nombreTienda}
                                onChange={(e) => setBranding({ ...branding, nombreTienda: e.target.value })}
                        ...
                        >
                            {savingBranding ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}
                        ...
                        <div className="relative group">
                            <div className="w-32 h-32 rounded-[2.5rem] bg-black/40 border-2 border-dashed border-white/10 flex items-center justify-center overflow-hidden group-hover:border-white/20 transition-all duration-500">
                                {logoPreview || branding.logo ? (
                                    <img src={logoPreview || branding.logo} className="w-full h-full object-cover" alt="Favicon preview" />
                                ) : (
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            <div className="space-y-2">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-black text-white uppercase tracking-widest">Borrar Registros</span>
                                </div>
                                <p className="text-zinc-500 text-xs leading-relaxed max-w-sm">
                                    Elimina todas las citas registradas y los usuarios tipo "cliente". Los administradores y barberos no se verán afectados.
                                </p>
                            </div>
