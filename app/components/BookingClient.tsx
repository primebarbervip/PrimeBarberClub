...
export default function BookingClient({ barberos = [], config }: Props) {
  ...
  const getHorasDisponibles = (barberoId: number | null, date: Date) => {
    ...
    while (current < endTime) {
      ...
      if (!yaPaso && (!esAlmuerzo || estaHabilitado) && !estaBloqueado) {
        slots.push(horaStr);
      }
          ...
          <div className="px-6 pt-12 pb-32 flex flex-col items-center text-center space-y-10 max-w-lg mx-auto">
            ...
            <div className="pt-6 w-full space-y-4">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-zinc-300">
                BarberShop Prime · Elite Service
              </p>
            </div>
          </div>
        )}
      ...
      <div className="pb-6">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button key={i} disabled={esPasado} onClick={() => onSelectDate(d)} className={`h-10 w-10 mx-auto rounded-full font-black text-[13px] flex items-center justify-center transition-all ${isSelected ? 'bg-zinc-900 text-white shadow-xl' : esPasado ? 'opacity-10' : 'text-zinc-800 hover:bg-zinc-100'}`}>
                {i + 1}
              </button>
            )
          })}
